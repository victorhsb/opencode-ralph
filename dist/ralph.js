...
var AGENT_TYPES = ["opencode", "claude-code", "codex", "copilot"];
function resolveCommand(cmd, envOverride) {
  if (envOverride)
    return envOverride;
  if (IS_WINDOWS) {
...
Options:
  ...
  --use-sdk           Use OpenCode SDK instead of subprocess execution (Phase 1 migration)
...
function parseRotationInput(raw) {
  ...
  for (const entry of entries) {
    ...
    if (!AGENT_TYPES.includes(agent)) {
      console.error(`Error: Invalid agent '${agent}' in rotation entry '${entry}'. Valid agents: ${AGENT_TYPES.join(", ")}`);
      process.exit(1);
    }
...
for (let i = 0;i < args.length; i++) {
  ...
  if (arg === "--agent") {
    const val = args[++i];
    if (!val || !AGENT_TYPES.includes(val)) {
...
function loadPluginsFromConfig2(configPath) {
...
function ensureRalphConfig(options) {
  ...
  if (options.filterPlugins) {
    const plugins = [
      ...loadPluginsFromConfig2(userConfigPath),
      ...loadPluginsFromConfig2(projectConfigPath),
      ...loadPluginsFromConfig2(legacyProjectConfigPath)
    ];
...
async function runSupervisorOnce(state, supervisorConfig, history, coderOutput, sdkClient) {
  try {
    ...
    const agentConfig = AGENTS[supervisorConfig.agent];
    await validateAgent(agentConfig);
    const cmdArgs = agentConfig.buildArgs(supervisorPrompt, supervisorConfig.model, {
      allowAllPermissions,
...
${stderrText}`.trim();
    if (exitCode !== 0) {
      return {
        ...
        noAction: false,
        rawOutput: combined,
        error: `${agentConfig.configName} supervisor run exited with code ${exitCode}`
      };
    }
    return parseSupervisorOutput(combined, supervisorConfig.noActionPromise, supervisorConfig.suggestionPromise, state.iteration);
  } catch (error) {
    return {
      ok: false,
      noAction: false,
...
async function runRalphLoop() {
  ...
  if (resuming) {
    ...
    if (existingState.supervisor) {
      ...
      supervisorAgent = existingState.supervisor.agent;
  ...
  if (supervisorConfig.enabled) {
    await validateAgent(AGENTS[supervisorConfig.agent]);
  }
...
`);
  ...
  if (supervisorConfig.enabled) {
    console.log(`Supervisor: ENABLED (${AGENTS[supervisorConfig.agent].configName}${supervisorConfig.model ? ` / ${supervisorConfig.model}` : ""})`);
  }
...
\uD83D\uDD04 Iteration ${state.iteration}${iterInfo}${minInfo}`);
    ...
    const rotationIndex2 = usingRotation ? ((state.rotationIndex ?? 0) % state.rotation.length + state.rotation.length) % state.rotation.length : 0;
    let currentAgent = state.agent;
    let currentModel = state.model;
    if (usingRotation) {
      const entry = state.rotation[rotationIndex2];
    ...
    try {
      if (sdkClient) {
        ...
        let lastPrintedAt = Date.now();
        let lastToolSummaryAt = 0;
        const compactTools = !verboseTools;
        ...
        const sdkToolCounts = sdkResult?.toolCounts ?? new Map;
        for (const [tool, count] of sdkToolCounts) {
          toolCounts.set(tool, (toolCounts.get(tool) ?? 0) + count);
        }
        exitCode = sdkResult?.exitCode ?? 1;
...
\u26A0\uFE0F  ${agentConfig2.configName} exited with code ${exitCode}. Continuing to next iteration.`);
      ...
      if (supervisorCfg?.enabled) {
        console.log(`
\uD83D\uDD75\uFE0F  Running supervisor (${supervisorCfg.agent}${supervisorCfg.model ? ` / ${supervisorCfg.model}` : ""})...`);
